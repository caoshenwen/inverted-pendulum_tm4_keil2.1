...
void Sensor_GetVal(float *values)
{
	...
	ICM20602_ReadIMU(&accgyroval[3],&accgyroval[0]);
	cnt ++;
	cnt %= 10;
	if(cnt==0) {
		/*Read Compass Data*/
		LIS3MDL_readMag(&mag_val[0]);
		RT_Info.mag_x=mag_val[0];
		RT_Info.mag_y=mag_val[1];
		RT_Info.mag_z=mag_val[2];
		
		/*Read Baro Data*/
		RT_Info.baro_height = SPL06001_readBaro();
	}
	for(i = 0; i<6; i++) {
		if(i < 3) {
			...
			values[i] = ((float) accgyroval[i]) / 16.38375f;
		}
	}	
...
{
	static float q[4];
	static float SensorValue[9];
	static float q0q0,q0q1,q0q2,q0q3,q1q1,q1q2,q1q3,q2q2,q2q3,q3q3;
	//float rolll, pitchh, yaww;
	
	Sensor_GetVal(SensorValue);
	
	AHRS_update_IMU(SensorValue[3] * PI/180, 
					SensorValue[4] * PI/180, 
					SensorValue[5] * PI/180,
					SensorValue[0], 
					SensorValue[1], 
					SensorValue[2]);	
	q[0] = q0;
	...
	Accel_Src[0] = SensorValue[0]/4096;
	Accel_Src[1] = SensorValue[1]/4096;
	Accel_Src[2] = SensorValue[2]/4096;
	
	RT_Info.rateRoll=-SensorValue[3] * PI/180; 
	RT_Info.ratePitch=SensorValue[4] * PI/180;
	RT_Info.rateYaw=-SensorValue[5] * PI/180;
	
	RT_Info.Roll = -(atan2f(2.0f*(q0q1 + q2q3),
							1 - 2.0f*(q1q1 + q2q2)))* 180/PI;
					   
    RT_Info.Pitch = safe_asinf(2.0f*(q0q2 - q1q3))* 180/PI;

    RT_Info.Yaw = -atan2f( 2.0f*q1q2 + 2.0f*q0q3, 
						  -2.0f*q2q2 - 2.0f*q3q3 + 1) * 180/PI;
}
...
void AHRS_update_IMU(float gx, float gy, float gz, float ax, float ay, float az) {
	...
	if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
		...
		halfvx = (q1 * q3 - q0 * q2)*(1);
		halfvy = (q0 * q1 + q2 * q3)*(1);
		halfvz = (q0 * q0 - 0.5f + q3 * q3)*(1);
